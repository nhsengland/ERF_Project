...
* West Yorkshire ICB Provider Codes (Acute only): RAE RCF RR8 RWY RXF
* Provider RJR dropped from control set due to missing Total data
***Data Importing Code 
clear
#delim ;
odbc load, exec(`"
...
gen WY = (Provider_Code == "RCF" | Provider_Code == "RWY" | Provider_Code == "RXF" | Provider_Code == "RAE" | Provider_Code == "RR8") 
gen ERF_Flag = date("02/02/2222", "MDY")
format ERF_Flag %tdnn/dd/CCYY
replace ERF_Flag = date("04/01/2023", "MDY") if Provider_Code == "RCF" | Provider_Code == "RWY" | Provider_Code == "RXF" | Provider_Code == "RAE" | Provider_Code == "RR8"
format ERF_Flag %tdnn/dd/CCYY
...
* SDID Analysis
local excelPath "Outputs/Main/SDID_Results_Non_Admitted_Pathways.xlsx"
...
foreach depvar in `depvars' {
    capture sdid `depvar' Group_ID Calendar_Month Treated, vce(bootstrap) seed(123456) reps(10) graph
        if _rc == 0 {
            ...
            local row = `row' + 1
            graph export "Outputs/Main/SDID_Results_Non_Admitted_Pathways_`depvar'.png", replace
        }
        else {
...
local excelPath "Outputs/Main/SDID_Results_Admitted_Pathways.xlsx"
putexcel set "`excelPath'", modify
putexcel A1 = "DepVar" B1 = "ATT" C1 = "SE" D1 = "Lower CI" E1 = "Upper CI" F1 = "SS"
local row = 2
preserve
    ...
    capture sdid `depvar' Group_ID Calendar_Month Treated, vce(bootstrap) seed(123456) reps(10) graph
        if _rc == 0 {
            ...
            putexcel A`row' = "`depvar'" B`row' = `att' C`row' = `se' ///
             D`row' = formula("=B`row' - (1.96 * C`row')") E`row' = formula("=B`row' + (1.96 * C`row')") F`row' = formula("=IF(AND(D`row' > 0, E`row' > 0), 1, IF(AND(D`row' < 0, E`row' < 0), 1, 0))")
            local row = `row' + 1
            graph export "Outputs/Main/SDID_Results_Admitted_Pathways_`depvar'.png", replace
        }
        else {
            display "sdid failed for DepVar=`depvar'. Skipping..."
        }
...
* Initialize Excel file path and headers
local excelPath "Outputs/Main/SDID_Results_Incomplete_Pathways.xlsx"
putexcel set "`excelPath'", modify
putexcel A1 = "DepVar" B1 = "ATT" C1 = "SE" D1 = "Lower CI" E1 = "Upper CI" F1 = "SS"
local row = 2
preserve
    ...
    capture sdid `depvar' Group_ID Calendar_Month Treated, vce(bootstrap) seed(123456) reps(10) graph
        if _rc == 0 {
            ...
            putexcel A`row' = "`depvar'" B`row' = `att' C`row' = `se' ///
             D`row' = formula("=B`row' - (1.96 * C`row')") E`row' = formula("=B`row' + (1.96 * C`row')") F`row' = formula("=IF(AND(D`row' > 0, E`row' > 0), 1, IF(AND(D`row' < 0, E`row' < 0), 1, 0))")
            local row = `row' + 1
            graph export "Outputs/Main/SDID_Results_Incomplete_Pathways_`depvar'.png", replace
        }
        else {
            display "sdid failed for DepVar=`depvar'. Skipping..."
        }
...
* Initialize Excel file path and headers
local excelPath "Outputs/Main/SDID_Results_Incomplete_Pathways_DTA.xlsx"
putexcel set "`excelPath'", modify
putexcel A1 = "DepVar" B1 = "ATT" C1 = "SE" D1 = "Lower CI" E1 = "Upper CI" F1 = "SS"
local row = 2
preserve
    ...
    capture sdid `depvar' Group_ID Calendar_Month Treated, vce(bootstrap) seed(123456) reps(10) graph
        if _rc == 0 {
            ...
            putexcel A`row' = "`depvar'" B`row' = `att' C`row' = `se' ///
             D`row' = formula("=B`row' - (1.96 * C`row')") E`row' = formula("=B`row' + (1.96 * C`row')") F`row' = formula("=IF(AND(D`row' > 0, E`row' > 0), 1, IF(AND(D`row' < 0, E`row' < 0), 1, 0))")
            local row = `row' + 1
            graph export "Outputs/Main/SDID_Results_Incomplete_Pathways_DTA_`depvar'.png", replace
        }
        else {
            display "sdid failed for DepVar=`depvar'. Skipping..."
        }
