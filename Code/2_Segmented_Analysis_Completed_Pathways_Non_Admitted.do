...
collapse (sum) Under_24_Weeks Over_52_Weeks Over_65_Weeks Total Total_Manual, by(Provider_Code Stage T_Code Yearmonth)
...
* Run SDID analysis
local excelPath "Outputs/Segmented/SDID_Results_Completed_Non_Pathways_Admitted.xlsx"
...
levelsof T_Code, local(T_Codes)
foreach code in `T_Codes' {
    preserve
    keep if T_Code == "`code'"
    gen Flag1 = 1
    ...
    foreach depvar in `depvars' {
        ...
        if _rc == 0 {
            local att = e(ATT)
            local se = e(se)
            putexcel A`row' = "`code'" B`row' = "`depvar'" C`row' = `att' D`row' = `se' ///
            ...
            local row = `row' + 1
            graph export "Outputs/Segmented/SDID_Completed_Pathways_Non_Admitted_`code'_`depvar'.png", replace
        }
        else {
            display "sdid failed for T_Code=`code', DepVar=`depvar'. Skipping..."
        }
    }
    restore
}
